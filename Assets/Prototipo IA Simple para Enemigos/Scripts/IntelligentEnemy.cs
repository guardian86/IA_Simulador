...
public class IntelligentEnemy : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        GameObject puntosCoberturaObjeto = GameObject.Find("Puntos de Cobertura");
        int childCount = puntosCoberturaObjeto.transform.childCount;
        puntosDeCobertura = new GameObject[childCount];
        for (int i = 0; i < childCount; i++)
        {
            puntosDeCobertura[i] = puntosCoberturaObjeto.transform.GetChild(i).gameObject;
        }
    ...
    private void AtacandoUpdate()
    {
        ...
        if (EnemigoEnRangoDeVision())
        {
            ...
            {
                //Debo disparar
                if (arma.Shoot(gameObject.transform.forward))
                {
                    //El arma se disparó
